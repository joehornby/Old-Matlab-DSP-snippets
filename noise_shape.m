function [output,noise] = noise_shape(x,ditherType,shapeNoise) % x is number of levels (first call) and sample (subsequent calls).
...
if isempty(NLEVELS)
    ...
    elseif isfinite(x) && ditherType >= 0 && ditherType <= 2 && islogical(shapeNoise)
        ...
        NLEVELS = x;
        DITHER = ditherType;
        A1 = 1.537;     % Values according to Lipshitz (***Change Value***).
        A2 = -0.8367;   % Try higher order (A3, A4..). Which is best? Limit?
        A3 = 0;
    ...
    if nargin == 1
        if SHAPENOISE == true
%             quantInput = x + (A1 * ZMINUS1 + A2 * ZMINUS2 + A3 * ZMINUS3);
